...
# Function to get random AWWW transition
...
apply_theme() {
    ...
    if [[ -f "$wallpaper" ]] && command -v awww &> /dev/null; then
        local transition_type=$(get_random_transition)
        local duration=$((1 + RANDOM % 3))  # Random duration between 1-3 seconds
        echo "Setting wallpaper with AWWW: $wallpaper"
        echo "Using random transition: $transition_type (${duration}s)"
        awww img "$wallpaper" --transition-type "$transition_type" --transition-duration "$duration"
        echo "Wallpaper set successfully"
    elif [[ -f "$wallpaper" ]]; then
        echo "AWWW not found, but wallpaper exists: $wallpaper"
        echo "Please install awww for wallpaper switching"
    else
...
reload_colors() {
    ...
    if command -v awww &> /dev/null; then
        current_wallpaper=$(awww query | grep -o 'image: [^[:space:]]*' | cut -d' ' -f2 | head -1)
    fi
